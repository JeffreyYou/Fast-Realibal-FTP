...
void CountThread(int sck, sockaddr_in hintM){
    ...
    while(notfinished){
        ...
        misspkt = to_string(currenttoken);
...
int main(int argc, char *argv[]){
    ...
    hintM.sin_port = htons(SERVER_PORT);
    ...
    if(askreceived>0){
        string nt(buf);
        cout <<"The number of nt is " << nt <<endl;
        numoftoken = stoi(nt);
    ...
    while(true){
        ...
        cout << "I got a packet from server whose token is " << token <<endl; 
        cout << "string is " << s;
