...
int client_send_name()
{
    if (recv_buffer[0] == 'g' && recv_buffer[1] == 'e' && recv_buffer[2] == 't')
    {
        ...
        if (n < 0)
            cout << "Server failed to send name to client" << endl;
    }
    return 0;
}
...
int client_send_packet_num(int total_packet)
{
    ...
    if (n < 0)
        cout << "Server failed to send packet number to client" << endl;
...
int send_lost_packet(int resend_token)
{
    ...
    if (n < 0)
        cout << "Server failed resend packet: " << resend_token << endl;
